...
class PCB{
	...
	float temp;
	float responseTime;
	PCB(string PName, string PState,int PID,long long int PCounter,
...
int main(){
    ...
    while(count>0){
        ...
        if(index>=0 && index<=readyQ.size()){
            ...
            currentTime+=readyQ[index]->BurstTime;
            readyQ[index]->BurstTime=0;
    ...
    <<endl;
	ofstream out("output31.txt");
	ofstream out1("output33.txt");
	ofstream out2("output32.txt");
